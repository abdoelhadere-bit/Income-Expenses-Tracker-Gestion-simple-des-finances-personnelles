...
$totalIncomes = $pdo->query("SELECT SUM(montant) as total FROM incomes")->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;

// Calcul du total des dépenses
$totalExpenses = $pdo->query("SELECT SUM(montant) as total FROM expenses")->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;

// Calcul du solde
$balance = $totalIncomes - $totalExpenses;

// Revenus du mois en cours
$currentMonthIncomes = $pdo->query("SELECT SUM(montant) as total FROM incomes WHERE MONTH(dates) = MONTH(CURRENT_DATE()) AND YEAR(dates) = YEAR(CURRENT_DATE())")->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;

// Dépenses du mois en cours
$currentMonthExpenses = $pdo->query("SELECT SUM(montant) as total FROM expenses WHERE MONTH(dates) = MONTH(CURRENT_DATE()) AND YEAR(dates) = YEAR(CURRENT_DATE())")->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;

// Données pour le graphique mensuel
$monthlyIncomes = $pdo->query("SELECT DATE_FORMAT(dates, '%Y-%m') as month, SUM(montant) as total FROM incomes GROUP BY DATE_FORMAT(dates, '%Y-%m') ORDER BY month DESC LIMIT 6")->fetchAll(PDO::FETCH_ASSOC);

$monthlyExpenses = $pdo->query("SELECT DATE_FORMAT(dates, '%Y-%m') as month, SUM(montant) as total FROM expenses GROUP BY DATE_FORMAT(dates, '%Y-%m') ORDER BY month DESC LIMIT 6")->fetchAll(PDO::FETCH_ASSOC);
